<script>
	...
	async function insertPneumonia() {
		if (
			...
			console.log(
				"Inserting Pneumonia...." + JSON.stringify(newPneumonia)
			);
	...
	async function BorrarPneumonia(name, year) {
		...
		const res = await fetch(
			"/api/v1/pneumonia-stats/" + name + "/" + year,
			{
				method: "DELETE",
			}
		).then(function (res) {
			...
			<Nav navbar>
				...
				<Dropdown>
					...
					<DropdownMenu end>
						...
						>
						<DropdownItem divider />
						<DropdownItem href="./#/graphpneumonia"
							>Pneumonia-Stats</DropdownItem
						>
						<DropdownItem divider />
				...
				<tbody>
					<tr>
						...
						<td
							><input
								type="number"
								bind:value={newPneumonia.year}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newPneumonia.ages_zero_fifty}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newPneumonia.ages_fifty_seventy}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newPneumonia.ages_seventy}
							/></td
						>
