<script>
	...
	async function insertcancerdeaths() {
		if (
			newcancerdeaths.country == "" ||
			newcancerdeaths.country == null ||
			newcancerdeaths.year == "" ||
			newcancerdeaths.year == null ||
			newcancerdeaths.ages_zero_fifty == "" ||
			newcancerdeaths.ages_zero_fifty == null ||
			newcancerdeaths.ages_fifty_seventy == "" ||
			newcancerdeaths.ages_fifty_seventy == null ||
			newcancerdeaths.ages_seventy == "" ||
			...
			console.log(
				"Inserting cancerdeaths...." + JSON.stringify(newcancerdeaths)
			);
			const res = await fetch("/api/v1/cancerdeaths-stats", {
				...
				if (res.status == 201) {
					...
					console.log(
						"ERROR There is already a data with that country and year in the database"
					);
				}
			});
		}
	...
	async function Borrarcancerdeaths(name, year) {
		const res = await fetch(
			"/api/v1/cancerdeaths-stats/" + name + "/" + year,
			{
				method: "DELETE",
			}
		).then(function (res) {
...
<main>
	...
	{#if $isAuthenticated}
		...
		<Alert
			color="success"
			isOpen={visibleMsg}
			toggle={() => (visibleMsg = false)}
		>
			{#if msg}
				<p>Correcto: {msg}</p>
			{/if}
		</Alert>
		<Button on:click={getPgAnt}>Página Anterior</Button>
		<Button on:click={getPgSig}>Página Siguiente</Button>
		{#await cancerdeaths}
			loading
		{:then cancerdeaths}
			<Table bordered>
				...
				<tbody>
					<tr>
						...
						<td
							><input
								type="number"
								bind:value={newcancerdeaths.year}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newcancerdeaths.ages_zero_fifty}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newcancerdeaths.ages_fifty_seventy}
							/></td
						>
						<td
							><input
								type="number"
								bind:value={newcancerdeaths.ages_seventy}
							/></td
						>
						...
						<tr>
							...
							<td align="center"
								><Button
									...
									color="warning"
									on:click={function () {
										window.location.href = `/#/cancerdeaths/${
											cd.country + "/" + cd.year
										}`;
									}}
								>
					...
					<tr>
						...
						<td
							><Button
								outline
								color="danger"
								on:click={BorrarcancerdeathsAll}
							>
								Borrar todo
							</Button></td
						>
